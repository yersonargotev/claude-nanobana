...
export class ImageGenerator {
    ai;
    modelName;
    static DEFAULT_MODEL = 'gemini-2.5-flash-image';
    ...
    static validateAuthentication() {
        ...
        throw new Error('ERROR: No valid API key found. Please set NANOBANANA_GEMINI_API_KEY, NANOBANANA_GOOGLE_API_KEY, GEMINI_API_KEY, or GOOGLE_API_KEY environment variable.\n' +
                ...
                try {
                    ...
                    if (response.candidates && response.candidates[0]?.content?.parts) {
                        for (const part of response.candidates[0].content.parts) {
                            ...
                            if (imageBase64) {
                                const filename = FileHandler.generateFilename(`${type}step${stepNumber}${request.prompt}`, 'png', // Stories default to png
                                0);
                                const fullPath = await FileHandler.saveImageFromBase64(imageBase64, outputPath, filename);
                                generatedFiles.push(fullPath);
            ...
            if (response.candidates && response.candidates[0]?.content?.parts) {
                ...
                for (const part of response.candidates[0].content.parts) {
                    ...
                    if (resultImageBase64) {
                        const filename = FileHandler.generateFilename(`${request.mode}_${request.prompt}`, 'png', // Edits default to png
                        0);
                ...
                if (!imageFound) {
                    console.error('DEBUG - No valid image data found in edit response parts');
                }
                // Handle preview if requested
                await this.handlePreview(generatedFiles, request);
                return {
        ...
        try {
            ...
            if (response.candidates && response.candidates[0]?.content?.parts) {
                ...
                for (const part of response.candidates[0].content.parts) {
                    ...
                    if (resultImageBase64) {
                        const filename = FileHandler.generateFilename(`remix_${request.prompt}`, 'png', 0);
                        const fullPath = await FileHandler.saveImageFromBase64(resultImageBase64, outputPath, filename);
                        generatedFiles.push(fullPath);
                        await this.handlePreview(generatedFiles, request);
                        return {
